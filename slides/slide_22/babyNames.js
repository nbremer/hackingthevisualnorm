pt.babyNames = pt.babyNames || {};

pt.babyNames.init = function(sectionID, girls) {
	
	//Remove any existing svgs
	d3.selectAll("#voronoi-baby-names svg").remove();
	d3.selectAll("#brush-baby-names svg").remove();

	pt.babyNames.hideVoronoi = true;
	...
	pt.babyNames.margin = {top: 20, right: 30, bottom: 30, left: 50};
	pt.babyNames.marginAll = {top: 20, right: 30, bottom: 30, left: 50};
	var margin = pt.babyNames.margin,
		marginAll = pt.babyNames.marginAll;

	pt.babyNames.width = $(".slides").width()*0.8 - margin.left - margin.right;
	pt.babyNames.heightBrush = 600 - margin.top - margin.bottom;
	pt.babyNames.heightAll = 100 - marginAll.top - marginAll.bottom;

	var width = pt.babyNames.width,
		heightBrush = pt.babyNames.heightBrush,
		heightAll = pt.babyNames.heightAll;

	//Create context SVG
	pt.babyNames.context = d3.select("#" + sectionID + " .babyNamesFinalContext").append("svg")
	...
	//Create focus SVG
	pt.babyNames.focus = d3.select("#" + sectionID + " .babyNamesFinalFocus").append("svg")
	...
	var marginLegend = {top: 15, right: 30, bottom: 10, left: 30},
		...
		heightLegend = 30;
	pt.babyNames.widthLegend = widthLegend;

	//Create color legend SVG
	pt.babyNames.colorLegend = d3.select("#" + sectionID + " .babyNamesFinalColorLegend").append("svg")
	...
	pt.babyNames.strokeLegend = d3.select("#" + sectionID + " .babyNamesFinalWidthLegend").append("svg")
	    .attr("width", widthLegend + marginLegend.left + marginLegend.right)
	    .attr("height", heightLegend + marginLegend.top + marginLegend.bottom)
	  .append("g")
	...
	pt.babyNames.update(girls);

}//init

pt.babyNames.update = function(girls) {

	var margin = pt.babyNames.margin,
		marginAll = pt.babyNames.marginAll,
		width = pt.babyNames.width,
		heightBrush = pt.babyNames.heightBrush,
		heightAll = pt.babyNames.heightAll,
		widthLegend = pt.babyNames.widthLegend;

	var context = pt.babyNames.context,
		focus = pt.babyNames.focus,
		colorLegend = pt.babyNames.colorLegend,
		strokeLegend = pt.babyNames.strokeLegend;
	...
	var voronoi = d3.geom.voronoi()
	    .x(function(d) { return xBrush(d.year); })
	    .y(function(d) { return yBrush(d.position); })
	    .clipExtent([[0, 0], [width, heightBrush]]);
	...
	function brushmove() {
		...
		voronoiGroup.selectAll("path")
			...
			.style("stroke-opacity", pt.babyNames.hideVoronoi ? 0 : 0.5)
...
pt.babyNames.hideCells = function() {
	pt.babyNames.hideVoronoi = true;
	d3.selectAll("#voronoi-baby-names .babyNamesFinalFocus .voronoiCells")
		.transition().duration(1000)
		.style("stroke-opacity", 0);
}//hideCells

pt.babyNames.showCells = function() {
	pt.babyNames.hideVoronoi = false;
	d3.selectAll("#voronoi-baby-names .babyNamesFinalFocus .voronoiCells")
		.transition().duration(1000)
